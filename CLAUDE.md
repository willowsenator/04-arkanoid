...
5. **`bricks.js`** — brick grid: `createGrid(options)` builds a row/col grid of brick rects, `findCollidingBrick` (first non-destroyed brick colliding with the ball), `allCleared`. Depends on `geometry.js` for collision.
...
7. **`main.js`** — the only file with DOM/canvas code: wires `Game` to the `<canvas>`, draws paddle/ball/bricks/heart-icon lives indicator, draws the "Game Over"/"You Win!" message directly on the canvas when `game.state.status` is `'gameover'`/`'win'`, drives the `requestAnimationFrame` loop, and binds keyboard (arrow keys / A-D to move, Enter/Space to restart when the game has ended), mouse, for input. Not required by tests — it runs only in the browser.
